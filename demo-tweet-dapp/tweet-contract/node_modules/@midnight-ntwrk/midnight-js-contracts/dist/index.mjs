import { ContractState as ContractState$1, signatureVerifyingKey, QueryContext as QueryContext$1, emptyZswapLocalState, decodeZswapLocalState, constructorContext, sampleSigningKey } from '@midnight-ntwrk/compact-runtime';
import { UnprovenOutput, UnprovenTransient, UnprovenInput, UnprovenOffer, VerifierKeyInsert, MaintenanceUpdate, signData, ContractCallsPrototype, VerifierKeyRemove, ReplaceAuthority, ContractState, ContractCallPrototype, communicationCommitmentRandomness, ContractDeploy, QueryContext, StateValue, ContractOperationVersionedVerifierKey, ContractOperationVersion, ContractMaintenanceAuthority, partitionTranscripts, PreTranscript, LedgerParameters } from '@midnight-ntwrk/ledger';
import { getZswapNetworkId, getRuntimeNetworkId, getLedgerNetworkId } from '@midnight-ntwrk/midnight-js-network-id';
import { parseCoinPublicKeyToHex, parseEncPublicKeyToHex, assertDefined, assertIsContractAddress, fromHex, assertUndefined, toHex } from '@midnight-ntwrk/midnight-js-utils';
import { UnprovenTransaction, getImpureCircuitIds, SucceedEntirely } from '@midnight-ntwrk/midnight-js-types';
 ...
 * limitations under the License.
 */
const toLedgerContractState = (contractState) => ContractState.deserialize(contractState.serialize(getRuntimeNetworkId()), getLedgerNetworkId());
const fromLedgerContractState = (contractState) => ContractState$1.deserialize(contractState.serialize(getLedgerNetworkId()), getRuntimeNetworkId());
const toLedgerQueryContext = (queryContext) => {
    const ledgerQueryContext = new QueryContext(StateValue.decode(queryContext.state.encode()), queryContext.address);
...
const createUnprovenLedgerDeployTx = (verifierKeys, sk, contractState, zswapLocalState, encryptionPublicKey) => {
    ...
    return [
        ...
        new UnprovenTransaction(zswapStateToOffer(zswapLocalState, encryptionPublicKey), undefined, new ContractCallsPrototype().addDeploy(contractDeploy))
    ];
};
const createUnprovenLedgerCallTx = (circuitId, contractAddress, initialContractState, zswapChainState, partitionedTranscript, privateTranscriptOutputs, input, output, nextZswapLocalState, encryptionPublicKey) => {
    ...
    return new UnprovenTransaction(zswapStateToOffer(nextZswapLocalState, encryptionPublicKey, {
        contractAddress,
        zswapChainState
    }), undefined, new ContractCallsPrototype().addCall(new ContractCallPrototype(contractAddress, circuitId, op, partitionedTranscript[0], partitionedTranscript[1], privateTranscriptOutputs, input, output, communicationCommitmentRandomness(), circuitId)));
};
...
const unprovenTxFromContractUpdates = (contractAddress, updates, contractState, sk) => {
    ...
    return new UnprovenTransaction(new UnprovenOffer(), undefined, new ContractCallsPrototype().addMaintenanceUpdate(signedMaintenanceUpdate));
};
...
const callContractConstructor = (options) => {
    const constructorResult = options.contract.initialState(constructorContext('initialPrivateState' in options ? options.initialPrivateState : undefined, parseCoinPublicKeyToHex(options.coinPublicKey, getZswapNetworkId())), ...('args' in options ? options.args : []));
    return {
...
async function createUnprovenCallTx(providers, options) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(createCallOptions(options, parseCoinPublicKeyToHex(providers.walletProvider.coinPublicKey, getZswapNetworkId()), contractState, zswapChainState, privateState), providers.walletProvider.coinPublicKey, providers.walletProvider.encryptionPublicKey);
    }
